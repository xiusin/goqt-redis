			...
			Dial: func() (conn redis.Conn, err error) {
				...
				if config.Auth != "" {
					if _, err := conn.Do("AUTH", config.Auth); err != nil {
						conn.Close()
						if isQt.(bool) {
							helper.ShowWarningMessage("认证错误", err.Error())
						}
						return nil, err
					}
				}
...
func RedisManagerRemoveKey(data RequestData) string {
	...
	go func() {
		serveId := getFromInterfaceOrFloat64ToInt(data["id"])
		var idx = 0
		for k, v := range connectionList {
			if v.ID == int64(serveId) {
				idx = k
				break
			}
		}
		channel.UpdateKeyCh <- channel.UpdateKeyChStruct{
			ServeId:  serveId,
			DbIdx:    getFromInterfaceOrFloat64ToInt(data["index"]),
			ServeIdx: idx,
			Key:      key,
			Action:   channel.SUB,
		}
...
func readConfigJSON() error {
	f, err := os.OpenFile(helper.UserHomeDir("rdm.db"), os.O_RDWR, os.ModePerm)
...
func writeConfigJSON() error {
	//os.O_TRUNC 清空内容
	f, err := os.OpenFile(helper.UserHomeDir("rdm.db"), os.O_CREATE|os.O_TRUNC|os.O_RDWR, os.ModePerm)
	if err != nil {
		return err
	}
	defer f.Close()
